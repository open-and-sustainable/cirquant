...
UMP_MAX_TABLES=${UMP_MAX_TABLES:-2}
...
echo "UMP tables: $UMP_MAX_TABLES (historical sheets only)"
echo
...
julia --project=. -e "push!(LOAD_PATH, \"src\"); using CirQuant; CirQuant.fetch_ump_weee_data(db_path=\"$DB_PATH\", max_tables=$UMP_MAX_TABLES)" 2>&1 | tee -a raw_test.log
